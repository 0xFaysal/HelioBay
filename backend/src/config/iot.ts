...
export const iotConfigSchema=z.object({
  ...
  if(v.NODE_ENV==='production' && v.ALLOW_DEVICE_SIMULATOR) ctx.addIssue({code:'custom',message:'Simulator is forbidden in production'});
  if(v.MQTT_URL) {
    let u:URL;try{u=new URL(v.MQTT_URL);}catch{ctx.addIssue({code:'custom',message:'Invalid MQTT URL'});return;}
    if(u.username||u.password||!['mqtt:','mqtts:'].includes(u.protocol)) ctx.addIssue({code:'custom',message:'Use mqtt/mqtts URL without embedded credentials'});
    if(v.MQTT_TLS_ENABLED!==(u.protocol==='mqtts:')) ctx.addIssue({code:'custom',message:'MQTT TLS setting and URL disagree'});
    if(v.NODE_ENV==='production' && (!v.MQTT_TLS_ENABLED||!v.MQTT_USERNAME||!v.MQTT_PASSWORD)) ctx.addIssue({code:'custom',message:'Production MQTT requires TLS and credentials'});
  }
});
