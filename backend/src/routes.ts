...
import {EnergyService}from'./modules/energy/service.js';import{energyRoutes}from'./modules/energy/controller.js';
export function mountApi(app: Express, db: Database, verify: TokenVerifier, payments = new PaymentService(db,null,null), engine?:ChargingEngine) {
  ...
  app.use('/api/v1/admin',auth,authorize('ADMIN'),energyRoutes(new EnergyService(db)));
}
