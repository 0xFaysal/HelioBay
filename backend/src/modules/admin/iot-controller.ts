...
export function iotAdminRoutes(engine:ChargingEngine) {
 ...
 const mutation=async(actorId:string,key:string,action:string,targetId:string,input:unknown,requestId:string,work:(tx:Prisma.TransactionClient)=>Promise<unknown>)=>db.$transaction(async tx=>{
  ...
  if(previous){const record=previous.after as {hash:string;result:Prisma.JsonValue};if(record.hash!==hash)throw new ApiError(409,'IDEMPOTENCY_CONFLICT','Key already used');return record.result;}
  const result=await work(tx);
  await tx.auditLog.create({data:{actorId,action,targetType:'IoTOperation',targetId,reason:body.reason,requestId:`iot:${key}`,after:json({hash,result,requestId})}});return result;
 });
 ...
 for(const action of ['stop','reconcile'] as const)r.post(`/charging-sessions/:sessionId/${action}`,async(req,res)=>{const b=confirmation.strict().parse(req.body);ok(res,await engine.stop(id.parse(req.params.sessionId),req.user!.id,'ADMIN_STOPPED',idempotencyKey.parse(req.headers['idempotency-key']),res.locals.requestId,true,action==='reconcile',b.reason),202);});
 ...
 r.post('/devices/:deviceId/commands',async(req,res)=>{
  ...
  ok(res,await mutation(actorId,key,'DEVICE_COMMAND',deviceId,b,res.locals.requestId,async tx=>{
   ...
   if(d.status!=='ONLINE'||!d.lastSeenAt||Date.now()-d.lastSeenAt.getTime()>engine.config.DEVICE_OFFLINE_TIMEOUT_SECONDS*1000)throw new ApiError(409,'DEVICE_OFFLINE','Device must be online');
   if(await tx.chargingSession.count({where:{deviceId,completedAt:null,status:{notIn:['COMPLETED','FAILED']}}})||await tx.bay.count({where:{deviceId,relayOn:true}}))throw new ApiError(409,'ACTIVE_SESSION','Stop and reconcile before maintenance');
 ...
 r.patch('/devices/:deviceId/thresholds',async(req,res)=>{
  ...
  ok(res,await mutation(req.user!.id,idempotencyKey.parse(req.headers['idempotency-key']),'DEVICE_THRESHOLDS',deviceId,b,res.locals.requestId,async tx=>{
   await tx.$queryRaw`SELECT id FROM "Device" WHERE id=${deviceId} FOR UPDATE`;
   if(await tx.chargingSession.count({where:{deviceId,completedAt:null,status:{notIn:['COMPLETED','FAILED']}}}))throw new ApiError(409,'ACTIVE_SESSION','Cannot change thresholds during charging');
 ...
 r.patch('/bays/:bayId/relay',async(req,res)=>{
  ...
  ok(res,await mutation(req.user!.id,idempotencyKey.parse(req.headers['idempotency-key']),'BAY_RELAY_CONFIG',bayId,b,res.locals.requestId,async tx=>{
   const bay=await tx.bay.findUniqueOrThrow({where:{id:bayId}});await tx.$queryRaw`SELECT id FROM "Device" WHERE id=${bay.deviceId} FOR UPDATE`;
   if(bay.relayOn||await tx.chargingSession.count({where:{bayId,completedAt:null,status:{notIn:['COMPLETED','FAILED']}}}))throw new ApiError(409,'ACTIVE_SESSION','Cannot remap an active relay');
 ...
 r.post('/faults/:faultId/resolve',async(req,res)=>{
  ...
  const result=await mutation(req.user!.id,idempotencyKey.parse(req.headers['idempotency-key']),'FAULT_RESOLVED',faultId,b,res.locals.requestId,async tx=>{
   const f=await tx.fault.findUniqueOrThrow({where:{id:faultId}});await tx.$queryRaw`SELECT id FROM "Device" WHERE id=${f.deviceId} FOR UPDATE`;
   if(await tx.chargingSession.count({where:{deviceId:f.deviceId,completedAt:null,status:{notIn:['COMPLETED','FAILED']}}}))throw new ApiError(409,'ACTIVE_SESSION','Reconcile active charging before clearing faults');
   return tx.fault.update({where:{id:faultId},data:{status:'RESOLVED',resolvedAt:new Date()}});
  });engine.bus.publish({type:'fault.resolved',data:result});ok(res,result);
 });
 return r;
}
