...
export class ChargingEngine {
  ...
  async start(userId:string,input:unknown,key:string,requestId:string,adminContext?:{actorId:string;reason:string}) {
    ...
    if(existing){if(existing.startRequestHash!==hash)throw new ApiError(409,'IDEMPOTENCY_CONFLICT','Start key was used for another request');return existing;}
    if(!this.publisher.ready())throw new ApiError(503,'DEVICE_GATEWAY_OFFLINE','Device gateway is unavailable');
    const session=await this.db.$transaction(async tx=>{
      const station=await tx.station.findFirstOrThrow({where:{OR:[{code:data.stationId},{id:data.stationId}]},include:{tariff:true}});
      ...
      const bay=await tx.bay.findFirstOrThrow({where:{OR:[{code:data.bayId},{id:data.bayId}],stationId:station.id,deviceId:device.id}});
      ...
      if(duplicate){if(duplicate.startRequestHash!==hash)throw new ApiError(409,'IDEMPOTENCY_CONFLICT','Start key was used for another request');return duplicate;}
      const user=await tx.user.findUniqueOrThrow({where:{id:userId}});
      if(user.status!=='ACTIVE')throw new ApiError(403,'ACCOUNT_INACTIVE','Account must be active');
      const vehicle=await tx.vehicle.findFirstOrThrow({where:{id:data.vehicleId,ownerId:userId}});
      if(station.status!=='ONLINE'||!station.isOpen||!station.tariff.active)throw new ApiError(422,'STATION_UNAVAILABLE','Station is closed or unavailable');
      if(device.status!=='ONLINE'||!device.lastSeenAt||Date.now()-device.lastSeenAt.getTime()>this.config.DEVICE_OFFLINE_TIMEOUT_SECONDS*1000)throw new ApiError(422,'DEVICE_OFFLINE','Device heartbeat is stale');
      if(!device.credentialRef||(this.secrets(device.credentialRef)?.length??0)<32)throw new ApiError(422,'DEVICE_NOT_PROVISIONED','Device credentials are not configured');
      if(device.dataSource!=='LIVE_HARDWARE'&&!(device.dataSource==='SIMULATOR'&&this.config.ALLOW_DEVICE_SIMULATOR))throw new ApiError(422,'NON_AUTHORITATIVE_METER','This source is not authorized for charging');
      if(!bay.enabled||!['AVAILABLE','PLUGGED'].includes(bay.status)||bay.relayOn)throw new ApiError(409,'BAY_UNAVAILABLE','Bay is unavailable');
      if(vehicle.connectorType!==bay.connectorType)throw new ApiError(422,'CONNECTOR_MISMATCH','Vehicle connector does not match this bay');
      if(!bay.plugConnected||!bay.lastTelemetryAt||Date.now()-bay.lastTelemetryAt.getTime()>this.config.DEVICE_OFFLINE_TIMEOUT_SECONDS*1000)throw new ApiError(422,'PLUG_REQUIRED','Connect the vehicle before starting');
      if(await tx.fault.count({where:{deviceId:device.id,status:{not:'RESOLVED'},OR:[{bayId:null},{bayId:bay.id}]}}))throw new ApiError(422,'BLOCKING_FAULT','Resolve the device or bay fault first');
      if(await tx.chargingSession.count({where:{completedAt:null,status:{notIn:['COMPLETED','FAILED']},OR:[{ownerId:userId},{bayId:bay.id},{vehicleId:vehicle.id}]}}))throw new ApiError(409,'ACTIVE_SESSION','An active or unreconciled session already exists');
      ...
      await tx.auditLog.create({data:{actorId:adminContext?.actorId??userId,reason:adminContext?.reason,action:'SESSION_START_REQUESTED',targetType:'ChargingSession',targetId:created.id,requestId,after:{reservedMinor:reserved.toString(),commandId:command.id}}});return created;
    });
    this.notify('command.pending',session);return session;
  }
  private async queueStop(tx:Prisma.TransactionClient,session:ChargingSession,reason:StopReason,actorId:string,key:string,requestId:string,retry=false,auditReason?:string) {
    ...
    if(previous&&!retry)return session;
    ...
    if(existing){if(existing.sessionId!==session.id)throw new ApiError(409,'IDEMPOTENCY_CONFLICT','Command key belongs to another session');return session;}
  ...
  async stop(id:string,actorId:string,reason:StopReason,key:string,requestId:string,admin=false,retry=false,auditReason?:string) {
    const result=await this.db.$transaction(async tx=>{const s=await this.lockSession(tx,id);if(!admin&&s.ownerId!==actorId)throw new ApiError(404,'NOT_FOUND','Session not found');return this.queueStop(tx,s,reason,actorId,key,requestId,retry,auditReason);});
    this.notify('command.pending',result);return result;
  }
  async dispatch() {
    if(!this.publisher.ready())return;
    ...
    for(const command of commands) {
      if(!command.payload||!command.device.credentialRef)continue;const key=this.secrets(command.device.credentialRef);if(!key)continue;
      const claim=await this.db.deviceCommand.updateMany({where:{id:command.id,status:'PENDING',OR:[{lastAttemptAt:null},{lastAttemptAt:{lt:new Date(Date.now()-2000)}}]},data:{lastAttemptAt:new Date(),attempts:{increment:1}}});if(!claim.count)continue;
      try{await this.publisher.publish(command.device.station.code,command.device.publicId,command.payload,key);await this.db.deviceCommand.update({where:{id:command.id},data:{publishedAt:new Date()}});}catch{/* Durable command remains pending until retry/timeout. */}
    }
  }
  private async finish(tx:Prisma.TransactionClient,session:ChargingSession,energy:bigint,failed=false) {
    ...
    if(!failed&&session.status!=='STOP_PENDING'&&session.status!=='INTERRUPTED')session=await transition(tx,session,'STOP_PENDING');
    ...
    await tx.auditLog.create({data:{actorId:session.ownerId,action:'SESSION_FINALIZED',targetType:'ChargingSession',targetId:session.id,requestId:`final:${session.id}`,after:receipt}});return finished;
  }
  async acknowledge(deviceId:string,message:Ack) {
    const cmd=await this.db.deviceCommand.findFirst({where:{id:message.commandId,deviceId}});if(!cmd||cmd.sessionId!==message.sessionId)return;
    if(!cmd.sessionId){await this.db.deviceCommand.updateMany({where:{id:cmd.id,status:'PENDING'},data:{status:message.accepted?'ACKNOWLEDGED':'FAILED',acknowledgedAt:new Date(),failureCode:message.failureCode}});this.bus.publish({type:message.accepted?'command.acknowledged':'command.failed',data:{commandId:cmd.id,deviceId}});return;}
    const session=await this.db.$transaction(async tx=>{
      let s=await this.lockSession(tx,cmd.sessionId!);if(s.completedAt)return s;
      const current=await tx.deviceCommand.findUniqueOrThrow({where:{id:cmd.id}});
      if(current.status==='ACKNOWLEDGED'||current.status==='FAILED')return s;
      ...
      if(cmd.type==='START') {
        if(!message.accepted&&!message.relayOn&&BigInt(message.energyMWh)===0n&&s.energyMWh===0n&&s.status==='START_PENDING') {
          s=await tx.chargingSession.update({where:{id:s.id},data:{stopReason:'SAFETY_FAULT'}});return this.finish(tx,s,0n,true);
        }
        if(message.accepted&&message.relayOn&&!late&&s.status==='START_PENDING') {s=await transition(tx,s,'CHARGING');await tx.bay.update({where:{id:s.bayId},data:{status:'CHARGING',relayOn:true}});return tx.chargingSession.update({where:{id:s.id},data:{startedAt:new Date()}});}
      ...
      if(!message.accepted){s=await transition(tx,s,'INTERRUPTED');return tx.chargingSession.update({where:{id:s.id},data:{reconciliationRequired:true}});}return s;
    });
    this.notify(message.accepted?'command.acknowledged':'command.failed',session,{commandId:cmd.id,session});if(session.completedAt)this.notify('session.stopped',session);
  }
  async telemetry(deviceId:string,t:Telemetry) {
    ...
    const result=await this.db.$transaction(async tx=>{
      let s=await this.lockSession(tx,found.id);if(s.completedAt)return s;
      ...
      if(s.lastTelemetryAt&&at<=s.lastTelemetryAt)return s;
      ...
      if(!t.relayOn&&s.status==='CHARGING'&&!s.stopReason)return this.queueStop(tx,s,'SAFETY_FAULT',s.ownerId,`stop:${s.id}`,`relay:${s.id}`);
      return s;
    });
    this.notify(result.completedAt?'session.stopped':'session.telemetry',result,{session:result,telemetry:t});
    if(result.lowCreditWarned&&!result.completedAt)this.notify('credit.warning',result,{sessionId:result.id,remainingMinor:(result.reservedMinor-result.costMinor).toString()});
  }
  private async raiseFault(tx:Prisma.TransactionClient,deviceId:string,bayId:string,code:string) {
    if(!await tx.fault.findFirst({where:{deviceId,bayId,code,status:{not:'RESOLVED'}}}))await tx.fault.create({data:{deviceId,bayId,code,message:'Device safety condition'}});
    await tx.bay.update({where:{id:bayId},data:{status:'FAULT'}});
  }
  async handle(deviceId:string,message:DeviceMessage) {
    if(message.kind==='ack')await this.acknowledge(deviceId,message);
    if(message.kind==='telemetry')await this.telemetry(deviceId,message);
    if(message.kind==='event') {
      const bay=await this.db.bay.findFirst({where:{deviceId,code:message.bayId}});if(!bay)return;
      ...
      if(session)await this.stop(session.id,session.ownerId,message.event==='EMERGENCY_STOP'?'EMERGENCY_STOP':'SAFETY_FAULT',`stop:${session.id}`,`event:${message.sequence}`,true);
    }
  }
  async sweep() {
    ...
    for(const device of stale) {
      await this.db.device.update({where:{id:device.id},data:{status:'OFFLINE'}});await this.db.station.update({where:{id:device.stationId},data:{status:'OFFLINE'}});
      ...
      for(const session of sessions)await this.db.$transaction(async tx=>{let s=await this.lockSession(tx,session.id);if(s.completedAt)return;s=await this.queueStop(tx,s,'DEVICE_OFFLINE',s.ownerId,`stop:${s.id}`,`offline:${s.id}`);s=await transition(tx,s,'INTERRUPTED');await tx.chargingSession.update({where:{id:s.id},data:{reconciliationRequired:true}});await tx.bay.update({where:{id:s.bayId},data:{status:'OFFLINE'}});});
    }
    const expired=await this.db.deviceCommand.findMany({where:{status:'PENDING',expiresAt:{lte:new Date()}}});
    for(const command of expired)await this.timeout(command);
    ...
    for(const s of waiting) {
      ...
      if(stop&&Date.now()>stop.expiresAt.getTime()+this.config.DEVICE_OFFLINE_TIMEOUT_SECONDS*1000) {
        await this.db.$transaction(async tx=>{let current=await this.lockSession(tx,s.id);if(current.completedAt||current.status!=='STOP_PENDING')return;current=await transition(tx,current,'INTERRUPTED');await tx.chargingSession.update({where:{id:current.id},data:{reconciliationRequired:true}});});
      }
    }
    const active=await this.db.chargingSession.findMany({where:{completedAt:null,status:'CHARGING'}});
    for(const s of active)if(Date.now()-(s.lastTelemetryAt??s.startedAt??s.createdAt).getTime()>this.config.DEVICE_OFFLINE_TIMEOUT_SECONDS*1000)await this.stop(s.id,s.ownerId,'DEVICE_OFFLINE',`stop:${s.id}`,`stale:${s.id}`,true);
    for(const s of active)if(Date.now()-(s.startedAt??s.createdAt).getTime()>=s.maxDurationSeconds*1000)await this.stop(s.id,s.ownerId,'MAX_DURATION_REACHED',`stop:${s.id}`,`duration:${s.id}`,true);
  }
  private async timeout(command:DeviceCommand) {
    const session=await this.db.$transaction(async tx=>{
      const s=command.sessionId?await this.lockSession(tx,command.sessionId):null;
      const changed=await tx.deviceCommand.updateMany({where:{id:command.id,status:'PENDING'},data:{status:'TIMED_OUT',failureCode:'ACK_TIMEOUT'}});if(!changed.count||!s||s.completedAt)return s;
      const next=await this.queueStop(tx,s,'DEVICE_OFFLINE',s.ownerId,`stop:${s.id}`,`timeout:${command.id}`);
      await transition(tx,next,'INTERRUPTED');await tx.bay.update({where:{id:s.bayId},data:{status:'OFFLINE'}});return tx.chargingSession.update({where:{id:s.id},data:{reconciliationRequired:true}});
    });
