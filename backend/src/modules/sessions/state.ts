...
export async function transition(tx: Prisma.TransactionClient, session: ChargingSession, to: SessionStatus) {
    ...
    if (!allowed[session.status]?.includes(to))
        throw new ApiError(409, 'INVALID_SESSION_TRANSITION', `Cannot transition ${session.status} to ${to}`);
    const updated = await tx.chargingSession.update({ where: { id: session.id }, data: { status: to } });
    await tx.sessionEvent.create({ data: { sessionId: session.id, type: 'STATE_CHANGED', data: { from: session.status, to } } });
    return updated;
}
