...
export function chargingBudget(reservedMinor:bigint,tariffMinorPerKwh:bigint,configuredMaxEnergy:bigint) {
 if(reservedMinor<=0n||tariffMinorPerKwh<=0n||configuredMaxEnergy<=0n)throw new Error('Positive authorization inputs required');
...
export function cappedChargingCost(energyMWh:bigint,tariffMinorPerKwh:bigint,reservedMinor:bigint) {
 const cost=costMinor(energyMWh,tariffMinorPerKwh);return cost<reservedMinor?cost:reservedMinor;
}
