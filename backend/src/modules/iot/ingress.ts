...
export class DeviceIngress {
  ...
  async receive(topicName:string,bytes:Buffer,retained=false) {
    ...
    const scope=parseTopic(topicName);if(!scope)return false;
    let decoded:z.infer<typeof envelope>;try{decoded=envelope.parse(JSON.parse(bytes.toString('utf8')));}catch{return false;}
    const device=await this.db.device.findUnique({where:{publicId:scope.device},include:{station:true}});
    if(!device||device.station.code!==scope.station||device.status==='DISABLED'||!device.credentialRef)return false;
    const secret=this.secrets(device.credentialRef);if(!secret||secret.length<32||!verifySignature(topicName,decoded.payload,decoded.signature,secret))return false;
    const parsed=deviceMessage.safeParse(decoded.payload);if(!parsed.success)return false;const message=parsed.data;
    const expected={telemetry:'telemetry',ack:'acks',event:'events',status:'status'}[message.kind];if(scope.channel!==expected)return false;
    if(message.dataSource!==device.dataSource || (message.dataSource==='SIMULATOR'&&!this.config.ALLOW_DEVICE_SIMULATOR))return false;
    if(retained && !(message.kind==='status'&&!message.online))return false;
    ...
    if(!will&&(age>60000||age< -5000))return false;
    const accepted=await this.db.$transaction(async tx=>{
      ...
      if(will) {if(current.bootId!==message.bootId)return false;await tx.device.update({where:{id:device.id},data:{status:'OFFLINE'}});await enqueue();return true;}
      if(BigInt(message.sequence)<=current.lastSequence)return false;
      if(message.kind!=='status' && current.bootId!==message.bootId)return false;
      let bay;
      if('bayId' in message) {bay=await tx.bay.findFirst({where:{code:message.bayId,deviceId:device.id,stationId:device.stationId}});if(!bay)return false;}
      ...
      if(message.kind==='status') {await tx.station.update({where:{id:device.stationId},data:{status:'ONLINE'}});await enqueue();return true;}
      if(message.kind==='telemetry'&&bay) {
        ...
        await tx.bay.update({where:{id:bay.id},data:{lastTelemetry:message,lastTelemetryAt:new Date(),plugConnected:plug.connected,relayOn:message.relayOn,...(!active?{status:!message.online?'OFFLINE':message.faultCodes.length?'FAULT':!bay.enabled?'DISABLED':message.relayOn?'FAULT':plug.connected?'PLUGGED':'AVAILABLE'}:{})}});
      ...
      await enqueue();return true;
    });
    if(!accepted)return false;
    if(message.kind==='status')this.bus.publish({type:'station.status',stationId:device.stationId,public:true,data:{stationId:device.stationId,deviceId:device.id,online:message.online,dataSource:message.dataSource}});
    if(message.kind==='telemetry')this.bus.publish({type:'bay.status',stationId:device.stationId,public:true,data:{stationId:device.stationId,bayId:message.bayId,plugConnected:plugState(message,thresholds(device.thresholds,(await this.db.bay.findFirst({where:{deviceId:device.id,code:message.bayId}}))?.thresholds)).connected,relayOn:message.relayOn,dataSource:message.dataSource}});
