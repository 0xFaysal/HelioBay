...
export interface CommandPublisher {
    ready(): boolean;
    publish(station: string, device: string, payload: unknown, key: string): Promise<void>;
}
export class MqttGateway implements CommandPublisher {
    ...
    start() {
        ...
        this.client.on('close', () => { this.subscribed = false; if (this.stopping || this.retry)
            return; const ms = Math.min(30000, 1000 * 2 ** Math.min(this.attempt++, 5)); this.retry = setTimeout(() => { this.retry = undefined; this.client?.reconnect(); }, ms); this.retry.unref(); });
    }
    ...
    async close() { this.stopping = true; if (this.retry)
        clearTimeout(this.retry); await this.client?.endAsync(true); }
}
