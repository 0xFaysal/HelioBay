...
export function verifySignature(topicName:string,payload:unknown,signature:string,key:string) {if(!/^[a-f0-9]{64}$/.test(signature))return false;return timingSafeEqual(Buffer.from(signature,'hex'),Buffer.from(sign(topicName,payload,key),'hex'));}
