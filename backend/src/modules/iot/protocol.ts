...
export const telemetrySchema = z.object({ ...base, kind: z.literal('telemetry'), bayId: identifier, sessionId: identifier.nullable(), online: z.boolean(), plugConnected: z.boolean(), relayOn: z.boolean(), batterySenseAvailable: z.boolean(), vehicleBatteryMv: z.number().int().min(0).max(1000000).nullable(), vehicleBatteryPercent: z.number().int().min(0).max(100).nullable(), batteryPercentageEstimated: z.literal(true), solarVoltageMv: z.number().int().min(0).max(1000000), solarCurrentMa: z.number().int().min(0).max(2000000), solarPowerW: z.number().int().min(0).max(1000000), solarEnergyMWh:integerText.optional(), chargingVoltageMv: z.number().int().min(0).max(1000000), chargingCurrentMa: z.number().int().min(0).max(2000000), chargingPowerW: z.number().int().min(0).max(1000000), energyMWh: integerText, stationEvEnergyMWh:integerText.optional(), stationBatteryPercent: z.number().int().min(0).max(100).nullable(), stationBatteryPowerW:z.number().int().min(-1000000).max(1000000).optional(), auxiliaryPowerW:z.number().int().min(0).max(100000).optional(), gridImportPowerW:z.number().int().min(0).max(1000000).optional(), gridExportPowerW:z.number().int().min(0).max(1000000).optional(), gridImportEnergyMWh:integerText.optional(), gridExportEnergyMWh:integerText.optional(), source: z.enum(['SOLAR', 'STORAGE', 'GRID']), faultCodes: z.array(identifier).max(8), final: z.boolean().default(false) }).strict().refine(v=>!(v.gridImportPowerW&&v.gridExportPowerW),'A station cannot import and export in the same interval');
