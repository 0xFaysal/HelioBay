...
describe.skipIf(!url||!mqttUrl)('real MQTT simulator to REST and WebSocket',()=>{
 ...
 it('reserves Credits, charges, unplugs, settles once and updates owner/admin',async()=>{
  ...
  try{
   ...
   await engine.dispatch();await owner.waitFor(m=>m.type==='command.acknowledged');await owner.waitFor(m=>m.type==='session.telemetry');await until(async()=>!!(await ctx.db.chargingSession.findUnique({where:{id}}))?.completedAt);
