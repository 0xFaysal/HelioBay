...
try {
 await db.$transaction(async tx=>{
  ...
  if(!station.isDemo)throw new Error('Simulator may provision only a demo station');
  ...
  if(device.stationId!==station.id)throw new Error('Device assignment mismatch');
  if(await tx.chargingSession.count({where:{stationId:station.id,completedAt:null,status:{notIn:['COMPLETED','FAILED']}}}))throw new Error('Resolve active sessions before provisioning');
