...
export function chargingReceipt(data: Snapshot, s: Session): ReceiptData {
  ...
  return {
    ...
    amount: credits(s.costMinor), equivalent: bdt(s.costMinor), status: s.state === "completed" ? "Settled from prepaid credits" : "In progress — not settled", sandbox: false,
    groups: [
