...
export const paymentSchema = z.object({ id, userId: id, amountMinor: minor.min(1000), status: z.enum(["pending", "validating", "verified", "failed", "cancelled", "expired", "risk-review", "reversed"]), sandbox: z.boolean(), provider: z.enum(["SSLCOMMERZ","LOCAL_SANDBOX"]).optional(), providerReference: z.string().max(200).optional(), createdAt: iso, verifiedAt: iso.optional(), requestId: id, demoOutcome: z.enum(["success", "failure", "cancel", "pending"]).optional(), submittedAt: iso.optional() });
