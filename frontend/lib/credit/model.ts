...
const minor = integer.max(1_000_000_000);
...
export const vehicleSchema = z.object({ id, ownerId: id, name: z.string().min(2), plate: z.string().min(3), capacityWh: integer.min(10000).max(200000), battery: percent, connector: z.enum(["CCS2", "Type 2"]), isDefault: z.boolean() });
export const stationSchema = z.object({ id, name: z.string().min(3).max(100), address: z.string().min(3), landmark: z.string(), lat: z.number().min(-90).max(90), lng: z.number().min(-180).max(180), deviceId: id, online: z.boolean(), priceMinor: minor.min(1), powerKw: z.number().min(1).max(350), solarPercent: percent, image: z.string().regex(/^\/images\/[\w.-]+$/), amenities: z.array(z.string()), openingHours: z.string(), distanceKm: z.number().nonnegative().optional() });
export const baySchema = z.object({ id, stationId: id, deviceId: id, number: integer.min(1), relayChannel: integer.min(1).max(32), connector: z.enum(["CCS2", "Type 2"]), enabled: z.boolean(), plugged: z.boolean(), fault: z.boolean() });
export const deviceSchema = z.object({ id, stationId: id, online: z.boolean(), lastSeen: iso, firmware: z.string(), stationBattery: percent, solarW: z.number().min(0).max(1_000_000), gridBackup: z.boolean(), gridExport: z.boolean(), outcome: z.enum(["success", "failure", "timeout"]) });
...
export const walletSchema = z.object({ userId: id, balanceMinor: minor });
export const ledgerSchema = z.object({ id, userId: id, kind: z.enum(["top-up", "charging-debit", "adjustment", "reversal"]), amountMinor: z.number().int().min(-1_000_000_000).max(1_000_000_000), balanceAfterMinor: minor, reference: id, reason: z.string(), status: z.literal("posted"), sandbox: z.boolean(), at: iso });
export const paymentSchema = z.object({ id, userId: id, amountMinor: minor.min(1000), status: z.enum(["pending", "verified", "failed", "cancelled"]), sandbox: z.literal(true), createdAt: iso, verifiedAt: iso.optional(), requestId: id, demoOutcome: z.enum(["success", "failure", "cancel", "pending"]).optional(), submittedAt: iso.optional() });
