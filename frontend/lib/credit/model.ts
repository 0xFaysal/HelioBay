...
export const paymentSchema = z.object({ id, userId: id, amountMinor: minor.min(1000), status: z.enum(["pending", "verified", "failed", "cancelled"]), sandbox: z.literal(true), providerReference: z.string().max(200).optional(), createdAt: iso, verifiedAt: iso.optional(), requestId: id, demoOutcome: z.enum(["success", "failure", "cancel", "pending"]).optional(), submittedAt: iso.optional() });
