...
test("promo discount applies to energy, not booking fee", () => {
  ...
  assert.equal(cost.discount, 39);
  assert.equal(cost.fee, 20);
  assert.equal(cost.estimate, 370);
});

test(
  "valid future booking receives the first free bay",
  () => assert.equal(validateBooking(station, vehicle, start, 60, [], now), "BAY01")
);
...
test("adjacent bookings do not conflict; cancelled bookings release capacity", () => {
  ...
  assert.equal(overlaps(start, 60, {
    ...b,
    status: "cancelled"
  }), false);
});
