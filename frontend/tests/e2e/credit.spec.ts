...
test("mobile top-up, live session and final receipt have no overflow",async({page})=>{
  ...
  await connect(page);await expect.poll(async()=> (await state(page)).sessions[0].state).toBe("charging");await page.screenshot({path:"artifacts/credit-live-390.png",fullPage:true});await expect.poll(()=>page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);await page.getByRole("button",{name:"Emergency stop",exact:true}).click();await expect(page.getByRole("alertdialog").getByRole("button",{name:"Confirm",exact:true})).toBeDisabled();await page.getByLabel("Type STOP to confirm").fill("STOP");await confirm(page);await expect(page.getByText("EMERGENCY_STOP",{exact:true})).toBeVisible();await page.screenshot({path:"artifacts/credit-receipt-390.png",fullPage:true});await expect.poll(()=>page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
});

test("admin stop, offline and sensor fault reach the owner receipt",async({page,context})=>{
  test.setTimeout(120000);await login(page);const admin=await context.newPage();await login(admin,true);await admin.goto("/admin/devices");await connect(page);await expect.poll(async()=> (await state(page)).sessions[0].state).toBe("charging");await admin.getByRole("button",{name:"Stop Charging",exact:true}).click();await confirm(admin);await expect(page.getByText("ADMIN_STOPPED",{exact:true})).toBeVisible();
  await page.goto("/charge");await page.getByRole("button",{name:"Start Charging",exact:true}).click();await expect.poll(async()=> (await state(page)).sessions[0].state).toBe("charging");await admin.getByRole("button",{name:"Simulate offline",exact:true}).click();await confirm(admin);await expect(page.getByText("DEVICE_OFFLINE",{exact:true})).toBeVisible();await admin.getByRole("button",{name:"Reconnect device",exact:true}).click();await confirm(admin);
  await page.goto("/charge");await page.getByRole("button",{name:"Start Charging",exact:true}).click();await expect.poll(async()=> (await state(page)).sessions[0].state).toBe("charging");await admin.getByRole("button",{name:"Inject sensor fault",exact:true}).click();await expect(page.getByText("FAULT",{exact:true})).toBeVisible();await admin.goto("/admin/faults");await expect(admin.getByRole("main")).toContainText("Simulated sensor fault");
});
