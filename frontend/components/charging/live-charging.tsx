...
export function LiveCharging({ id }: { id: string }) {
  ...
  const estimatedMinutes = estimateRemainingMinutes((vehicle?.capacity ?? 60) * 1000 / (isDemo ? network.pricing.demoScalingFactor : 1), stale ? null : telemetry?.carBatteryPercent ?? null, telemetry?.carBatteryVoltage ?? null, telemetry?.chargingCurrent ?? null, network.pricing.taperFactor, session.targetBattery ?? 100);
  ...
  return <>
    ...
    <div className="dashboard-grid"><section className="panel text-center"><div className="panel-top"><h2 className="panel-title">{vehicle?.name ?? "Your vehicle"}</h2><BatteryCharging size={21} className="text-green-700" /></div>
      <div className={`battery-ring ${charging ? "charging-active" : ""}`} role="img" aria-label={`Estimated battery ${Math.round(session.battery)} percent`}><svg viewBox="0 0 230 230"><circle cx="115" cy="115" r="101" fill="none" stroke="#edf2ee" strokeWidth="9" /><circle cx="115" cy="115" r="101" fill="none" stroke="#00c865" strokeWidth="9" strokeLinecap="round" strokeDasharray={`${session.battery / 100 * 634.6} 634.6`} /></svg><div className="battery-ring-text"><strong><AnimatedNumber value={session.battery} /><span className="text-2xl muted">%</span></strong><small>{completed ? "Final estimate" : stale || session.status === "fault" ? "Last known estimate" : "Estimated battery"}</small></div></div>
      ...
      { icon: Zap, label: isDemo ? "EV-equivalent energy" : "Delivered energy", value: `${session.energy.toFixed(3)} kWh` },
    ...
    ].map(m => <div key={m.label}><m.icon size={17} className="text-green-700 mb-3" /><p className="text-[10px] muted">{m.label}</p><strong className="text-xl font-medium mt-1 block">{m.value}</strong></div>)}</div><div className="data-row mt-5"><span>{completed ? "Final bill" : "Running bill"}</span><strong className="text-2xl">{money(bill)}</strong></div><div className="data-row"><span>MOSFET / charger</span><strong>{!stale && device?.online ? device.mosfetOn ? "ON" : "OFF" : "Unknown / stale"}</strong></div>
