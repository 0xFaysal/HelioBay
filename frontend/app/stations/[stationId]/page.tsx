...
export async function generateMetadata(
  ...
  const s = stationService.get((await params).stationId);

  return {
    title: s?.name ?? "Station not found",
    description: s ? `${s.name} · ${s.address} · ${s.power} kW charging. Reserve a solar-powered bay.` : undefined,
...
export default async function Page(
  ...
  const station = stationService.get((await params).stationId);
        ...
        <div className="detail-layout">
          <div>
            ...
            <div className="flex flex-wrap justify-between items-center gap-3 my-6">
              <span className="text-xs"><span className={`status-dot ${!station.online ? "!bg-red-500" : ""}`} />{station.online ? "Station online" : "Temporarily offline"}· Open 24 hours</span>
              <a
            ...
            <div className="data-row">
              ...
              <span>Available · Automatic failover</span>
            </div>
            <div className="data-row">
              <span>Device</span>
