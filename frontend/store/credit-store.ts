...
import { withDemoAccount } from "@/lib/credit/demo-account";
...
export const useCreditStore = create<State>()(persist(set => ({
  ...
  setAccount: account => {
    identity.setState({ account: isDemo && account?.demo ? account : null });
    set(state => {
      const accountChanged = state.account?.id !== account?.id;
      const data = isDemo && account ? withDemoAccount(state.data, account) : !isDemo && accountChanged ? seed("1970-01-01T00:00:00.000Z", true) : state.data;
      return { account, data, error: accountChanged ? "" : state.error, ...(!isDemo && accountChanged ? { connection: "Connecting" } : {}) };
    });
  },
}), {
