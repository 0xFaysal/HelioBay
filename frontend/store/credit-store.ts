...
export const useCreditStore = create<State>()(persist(set => ({
  ...
  setAccount: account => { identity.setState({ account: isDemo && account?.demo ? account : null }); set(state => ({ account, error: state.account?.id !== account?.id ? "" : state.error, ...(!isDemo && state.account?.id !== account?.id ? {data: seed("1970-01-01T00:00:00.000Z",true), connection: "Connecting"} : {}) })); },
}), {
